...
public class UOFileReaderSystem {
    ...
    public void loadFiles() {
        ...
        try {
            final var mapPath = Paths.get(properties.files().dataFileRoot() + File.separator + "map0.mul");
            this.map = Map.open(mapPath);
            log.info("Map loaded successfully!");

            Path staidx0 = Paths.get(properties.files().dataFileRoot() + File.separator + "staidx0.mul");
            Path statics0 = Paths.get(properties.files().dataFileRoot() + File.separator + "statics0.mul");
